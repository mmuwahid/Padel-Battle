...
## NEXT SESSION (S105) — START HERE
**Last session:** S104 (2026-09-15 → 2026-09-16) — **1 commit (`a6ea8c5`, test-only — no app code, SW still v245), 3 Supabase migrations.** Hosting "outage" was **Supabase free-tier auto-pause** (`INACTIVE` after ~7 days idle), not Vercel — restored to `ACTIVE_HEALTHY`. **#154 CLOSED** — season creation threw `PGRST203` because the DB held two `create_season` overloads (`s093` used `CREATE OR REPLACE` while *reordering* params; Postgres keys on argument **types**, so it created a second function instead of replacing `s081`'s, and PostgREST binds by **name** → ambiguous). Dropped the stale S081 signature (`s104_drop_stale_create_season_overload`); broken since 2026-06-22, unnoticed because no season had been created since. **#155 FILED + FIXED (CRITICAL auth bypass)** — `is_platform_admin()` returned `NULL` (not `false`) for anon callers; `NOT NULL` is not TRUE, so `IF NOT admin_has_permission(...) THEN RAISE` never fired and every admin RPC fell straight through into its privileged body. All 11 admin RPCs were exploitable by any authenticated user against any league; 6 season RPCs additionally by pure anon. Fixed with `COALESCE(..., false)` at source + aggregate (`s104_fix_null_permission_bypass`), then hardened with null-caller guards on the 6 season RPCs (`s104_null_caller_guards_season_rpcs`) and locked behind a committed regression test (`supabase/tests/auth-gate-regression.sql`, PR #157) — **#155 closed**. **#156 FILED** — iOS 27 readiness audit (research only; do NOT rush the iOS 27 SDK for v1). Web unchanged: SW v245, main `373442f`.
**Prior session:** S103 (2026-07-03) — **2 commits (`12d489e` RevenueCat client-only wiring, `373442f` real iOS key), no SW change (config/native only).** iOS in-app-purchase config **COMPLETE end-to-end**: ASC app `PadelHub - Padel Game Tracker` (bundle `com.mohammedmuwahid.padelhub`, SKU `padelhub`), subscription group `PadelHub Pro` (Group ID `22206579`) + `PadelHub Pro Monthly` $4.99 + `PadelHub Pro Annual` $34.99 (both Missing Metadata — localization deferred). RevenueCat products entered **manually** (no ASC API key → auto-import greyed out); entitlement `pro`, offering `default` (Current, mapped to `$rc_monthly`/`$rc_annual`); real iOS key `appl_oqPyveTwMACTVTdlHUJoArMAdzN` in the `ios` slot of `src/revenuecat.js`. **Android still on Test Store key.** **7-day trial deferred** by user. **Play Store setup BLOCKED** — Google Play account not verified (`Create app` greyed out); needs device-access check + identity/phone verification (~1–3 day Google approval). Main `373442f`.
**Prior session:** S102 — Android signed AAB (`app-release.aab` v1) + Play listing + FCM deferred (`fbb7040`).

### 🎯 S105 PRIORITY — settle the Supabase pause, then Play verification unblocks RevenueCat
0a. ~~**Decide the Supabase auto-pause fix**~~ — **SETTLED (S104, USER):** Pro upgrade **rejected**, keep-warm cron **rejected** — "the app will be more active now as we will start playing again", so organic traffic is the keep-warm. ⚠️ Residual risk: if play stops for 7+ days before launch the backend pauses again; re-open this if the app goes quiet or on launch week.
0b. ~~**Optional #155 hardening**~~ — **ITEMS B + C DONE (S104).** B: `IF auth.uid() IS NULL THEN RAISE` guards added to all 6 season RPCs (migration `s104_null_caller_guards_season_rpcs`), placed *before* the object lookup so anon can't probe season existence. C: `supabase/tests/auth-gate-regression.sql` merged (PR #157, `a6ea8c5`) — 22/22 denial checks pass, owner path intact. **#155 CLOSED.** Still deferred by choice: **Item A** (revoke `anon` EXECUTE) — needs a grants audit first, because `REVOKE … FROM anon` is a silent no-op if the grant went to `PUBLIC`, and the naive fix locks out admins unless `authenticated` is re-granted in the same transaction. Also dropped: the `platform_admins` table idea (hardcoded UUID is fine at this scale).
1. **Google Play account verification** (USER, BLOCKING) — sign in to Play Console mobile app on an Android phone (device check) + complete identity/phone verification (Google approval ~1–3 days). Nothing on Play proceeds until `Create app` unlocks.
2. **Once verified — Play RevenueCat chain:** create Play app → Setup → API access → create service-account + download JSON → grant Play Developer perms → upload JSON to RevenueCat "New Play Store configuration" (package `com.mohammedmuwahid.padelhub`) → Save.
3. **Create Play subscription** `padelhub_pro` with base plans `p1m` ($4.99) / `p1y` ($34.99) → add RevenueCat Play products → attach to `pro` entitlement → assign to `$rc_monthly`/`$rc_annual` Play rows in `default` offering → **provide `goog_…` key to swap into the android slot of `src/revenuecat.js`.**
4. **ASC subscription-group Localization** (English U.S., display name `PadelHub Pro`) — required before iOS build submission.
5. **Wire tier-limit enforcement** using `isPro` (Free 1 league / 1 season / 5 player invites; Pro unlimited) — currently copy only, flagged in `MembershipView.jsx`.
6. **Sandbox device smoke-test** of the iOS purchase flow (needs a device).
7. **Redo the Play Store graphics** (S102 rejected) + capture 2+ phone screenshots (ranking, log-match, player profile, pairs, tournament bracket).
8. **Decide #129 v2**: full ~10-capability matrix, per-season overrides, member-grantable perms.
9. **Set up padelhub.app email addresses** (support@, privacy@, legal@) — placeholders in legal pages.
10. (Cleanup) Stale `.claude/launch.json` configs (`clone-dev`, `padel-dev-cleanup`, `mockup-static`) still point at the old `/tmp` path; `hardening-dev` + `clone-dev-user` point at `C:/Users/User/...`; `clone-dev-unhoec03` (S101) is the working config for THIS PC.
11. (Optional) Test infra from #137 (#1 unit, #2 E2E, #3 Storybook, #4 TS migration) — deferred, lower priority than launch.
12. (Cleanup) **Fix the clone path in root `CLAUDE.md`** — Cold Start step 5 says `C:/Users/User/dev/Padel-Battle`, but on THIS PC the repo is at `C:/Users/UNHOEC03/dev/Padel-Battle`.
13. **iOS 27 (#156)** — after v1 ships, device smoke-test WebKit scroll anchoring (leaderboards, match history, notification list) + subpixel inline layout (Syne/DM Mono typography). Affects live PWA users today and costs only testing time. UIScene migration is **v1.1**, bundled with #6 Universal Links.
...
### S104 outcomes (this session — archived)
- [x] **Restored hosting** — Supabase project was `INACTIVE` (free-tier auto-pause after ~7 days idle) → restored to `ACTIVE_HEALTHY`. Vercel was never suspended; `"live": false` + `403` on unpause are hobby-plan artefacts.
- [x] **#154 closed** — dropped the stale `create_season(uuid, text, date, uuid, text, text, text, date)` overload (migration `s104_drop_stale_create_season_overload`) + `NOTIFY pgrst, 'reload schema'`. Verified: exactly one overload remains, arg names match `SeasonManagement.jsx:96-108` exactly, owner-create returns a new season id (run in a transaction and rolled back), `end_date < start_date` still rejected. Whole-schema audit found no other duplicate overloads.
- [x] **#155 filed + fixed** — migration `s104_fix_null_permission_bypass`: `COALESCE(auth.uid() = '<uuid>', false)` in `is_platform_admin()` + `COALESCE(..., false)` wrapping the whole `admin_has_permission()` expression. Verification matrix all green: anon `false`, authenticated outsider `false`, cross-league owner `false`, real owner `true`; live anon `POST rpc/create_season` → `401`/`42501`; `delete_season` + `set_season_roster` as anon and `end_season` as authenticated outsider all raise `42501` (rolled back); owner create still succeeds. Production data verified unchanged after all testing.
- [x] **Audited every other boolean auth helper** — `is_league_admin_or_owner` uses `EXISTS`, which is strict and never returns NULL, so it is safe as written; remaining boolean functions are `IMMUTABLE` scoring/validation helpers with no auth role. The NULL-bypass class is confined to the two functions already fixed.
- [x] **Corrected the #155 blast radius publicly** — first write-up said all 11 RPCs were anon-exploitable; a grants + null-caller-guard audit showed 6. Posted a correction comment rather than leaving the overstatement.
- [x] **#156 filed** — iOS 27 readiness audit. Verified project state: Capacitor **8.4.1** (needs 8.5+ for UIScene), no `SceneDelegate.swift`, no `UIApplicationSceneManifest`, `UILaunchStoryboardName` present (new mandatory launch-screen key already satisfied). Confirmed **no** iOS 27 changes to Face ID/passkeys/WebAuthn (**#124 unaffected**), Web Push, home-screen install, or `setAppBadge`.
- [x] Added S104 lessons to `tasks/lessons.md` (3 mistakes + 4 validated patterns).
- [ ] ⚠️ **My mistake, logged:** fired a live anon `create_season` at the REAL league to prove the bypass — it returned `200`, created a real season, and auto-deactivated `Season 1`. Cleaned up (probe season deleted after verifying 0 matches / 0 roster; `Season 1` restored to active; all 4 leagues verified). Rule now: never send a mutating request to production to test an auth boundary — use `BEGIN…ROLLBACK` or a non-resolving ID.
- [x] **Supabase auto-pause decision** — USER rejected both Pro ($25/mo) and a keep-warm cron; relying on resumed real-world play as organic keep-warm. Risk accepted and logged.
- [x] **#155 hardening items B + C shipped** — B: migration `s104_null_caller_guards_season_rpcs` adds `v_uid uuid := auth.uid()` + `IF v_uid IS NULL THEN RAISE … USING ERRCODE='42501'` as the first statement of all 6 season RPCs, *ahead of* the season lookup. Post-apply verification: all 6 signatures byte-identical (no #154-style duplicate overload) and the whole-schema duplicate check is empty. C: `supabase/tests/auth-gate-regression.sql` (PR #157, squash-merged as `a6ea8c5`) — 3 tiers (anon / authenticated outsider / owner) × 11 admin RPCs; ran green against production, 22/22 denials, owner `create_season` still returns an id, nothing persisted. **#155 closed.**
- [ ] **#155 Item A deferred (deliberate)** — revoking `anon` EXECUTE on the 11 admin RPCs is now defence-in-depth, not the fix. Do a `information_schema.role_routine_grants` audit first: if the grant is to `PUBLIC`, `REVOKE … FROM anon` does nothing, and revoking from `PUBLIC` without re-granting `authenticated` in the same transaction locks out every admin.

### S103 outcomes (archived)
- [x] **RevenueCat client wiring** (`12d489e`) — native-guarded `src/revenuecat.js` bridge (entitlement `pro`, offering fetch, buy/restore, customer-info listener) + `SubscriptionContext`/`SubscriptionProvider` (`useSubscription()`, isPro, packages) + `MembershipView.jsx` paywall consuming live packages by `$rc_monthly`/`$rc_annual` + provider mount in `main.jsx`.
- [x] **ASC app record + subscriptions** — `PadelHub - Padel Game Tracker`, group `PadelHub Pro` (ID `22206579`), Monthly $4.99 + Annual $34.99.
- [x] **RevenueCat objects** — products added manually, `pro` entitlement (deleted immutable auto one first), `default` offering edited in place (Lifetime dropped, mapped to `$rc_monthly`/`$rc_annual`, Current).
- [x] **Real iOS key wired** (`373442f`) — `appl_oqPyveTwMACTVTdlHUJoArMAdzN` in the `ios` slot; lint/build green; both commits pushed.
- [ ] ⚠️ **Android key still Test Store** — swap real `goog_…` once Play products connect (S104).
- [ ] **7-day free trial** — deferred by user (add Introductory Offers later).
- [ ] **Play Store** — BLOCKED on Google account verification (device + identity).
- [ ] **iOS purchase flow** — NOT device-tested (needs sandbox device).
