...
- [x] **Sign in with Apple — DONE + verified end-to-end (S092, 2026-06-21).** Apple provider enabled in Supabase (Auth → Providers → Apple). Verified: the Supabase `authorize?provider=apple` endpoint 302-redirects to `appleid.apple.com` with the correct `client_id` + callback. Config: **Services ID** `com.mohammedmuwahid.padelhub.signin`, **Key ID** `NTSQJCBXXH`, **Team ID** `9M6M6A8B6V`, **.p8** at `C:\Users\User\Downloads\AuthKey_NTSQJCBXXH.p8` (BACK THIS UP — only downloadable once). **⚠️ The Supabase "Secret Key" is a JWT that EXPIRES 2026-12-18 (Apple's 6-month max) — Apple login will silently break after that until regenerated.** Regenerate with the saved script `gen-apple-secret.cjs` (run `node gen-apple-secret.cjs`, paste new token into Supabase → Apple → Secret Key). For the native iOS build, also add the bundle ID `com.mohammedmuwahid.padelhub` to Supabase "Client IDs" + add `@capacitor-community/apple-sign-in`.
- [ ] _(superseded)_ ~~Sign in with Apple — DASHBOARD CONFIG still pending (#121).~~ The button is already wired in `AuthGate.jsx` (`handleAppleSignIn` → `supabase.auth.signInWithOAuth({provider:'apple'})`) and the Apple logo renders correctly (verified S092). But it will **error until the Apple provider is enabled in the Supabase dashboard**: Authentication → Providers → Apple → add the **Services ID** + **Team ID** (`9M6M6A8B6V`) + **Key ID** + **.p8 private key** generated in the Apple Developer portal, and register the Supabase callback URL as a Return URL. This is a credentials/console step (cannot be done from code). Until then, keep Google as the working third-party option. **For the native iOS build, also add `@capacitor-community/apple-sign-in`** so the in-app flow uses the native Apple sheet rather than a web redirect.
- [ ] **Face ID / biometric login (#124).** Requires the native wrap — there is no reliable Face ID API inside a PWA. Plan: after Capacitor scaffolding, add a biometric plugin (`@capgo/capacitor-native-biometric` or `capacitor-native-biometric`) that, after a successful password/OAuth sign-in, securely stores the Supabase **refresh token** in the iOS Keychain / Android Keystore and gates its retrieval behind a Face ID / Touch ID / fingerprint prompt on subsequent cold opens. On launch: if a biometric credential exists → prompt → on success restore the session via `supabase.auth.setSession(refreshToken)`; on failure or no credential → fall back to the normal login screen. **Blocked on:** (a) Capacitor scaffold existing, (b) iOS build environment (§0). Pure web/PWA cannot satisfy this — do not attempt before the wrap.
