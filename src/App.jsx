...
function AppContent({leagueId,user,leagues,leagueHandlers}){
  ...
  // Push-notification subsystem (Issue #2 extraction). Declared after showToast
  // so the hook can surface failures through the toast system. Owns the push
  // subscription state, per-type preference toggles, and Edge-Function senders.
  const {
    pushSubscribed,
  ...
  return (
    ...
    <div style={{background:BG,minHeight:"100vh",paddingBottom:"calc(78px + env(safe-area-inset-bottom, 0px))",fontFamily: "var(--font)",color:TX}}>
      ...
      {/* Issue #15 + #43 (S058): paint html/body to the same color as the header/content so rubber-band overscroll at the page top AND the safe-area strip BELOW the bottom nav reveal a color identical to the rest of the app — no visible seam. S092 #130: changed #0d0d14 → #0a0a0f to match the unified header/nav/content surface (the old #0d0d14 left a lighter band in the bottom safe-area zone below the floating nav). The body bg paint is the actual fix; the previous `overscroll-behavior-y:none` was defensive and is now removed to restore native iOS rubber-band + momentum scrolling per #43. `-webkit-overflow-scrolling:touch` re-enables iOS momentum on legacy WebKit (no-op on iOS 13+).
              ...
              <div style={{position:"relative",display:"inline-flex",alignItems:"center",flexShrink:0}}>
                <select className="spill" value={selectedSeason||""} onChange={e=>setSelectedSeason(e.target.value)}
                  ...
                  {/* S089 #126: label is just the season name (no '(active)'/dot) —
                      ...
                      muted gray. Consistent across every season dropdown. */}
                  {seasons.map(s=><option key={s.id} value={s.id} style={{color:s.active?"#fff":"#9090a4"}}>{s.name}</option>)}
                </select>
