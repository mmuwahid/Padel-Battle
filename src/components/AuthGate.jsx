...
export function AuthGate({children}){
  ...
  // S089 #121: Sign in with Apple — required by App Store review when any other
  // third-party login is offered. Needs the Apple provider configured in the
  // Supabase dashboard (Service ID + key); the button is wired and ready.
  ...
  if (authMode === "recovery" || recoveryUser) {
    return (
      <div className="lscreen">
        <div className="lbg"/>
        <div className="lhero">
          <div className="llogobox"><PadelHubMark size={80}/></div>
    ...
    return (
      <div className="lscreen">
        ...
        <div className="lhero">
          <div className="llogobox"><PadelHubMark size={80}/></div>
          <div className="lbrand">Padel<span className="accent">Hub</span></div>
          {/* S089 #121: tagline removed per request for a cleaner login. */}
        </div>
        ...
        {authMode === "signin" && (
          <form className="lform" onSubmit={handleSignIn}>
            ...
            <button type="button" className="gbg2" onClick={handleAppleSignIn}>
              <svg width="17" height="17" viewBox="0 0 24 24" fill="currentColor" style={{flexShrink:0}}><path d="M17.05 12.04c-.03-3.16 2.58-4.68 2.7-4.75-1.47-2.15-3.76-2.45-4.57-2.48-1.95-.2-3.8 1.15-4.79 1.15-.98 0-2.5-1.12-4.12-1.09-2.12.03-4.08 1.23-5.17 3.13-2.2 3.82-.56 9.48 1.58 12.58 1.05 1.52 2.3 3.22 3.94 3.16 1.58-.06 2.18-1.02 4.09-1.02 1.9 0 2.45 1.02 4.12.99 1.7-.03 2.78-1.55 3.82-3.08 1.2-1.77 1.7-3.48 1.72-3.57-.04-.02-3.3-1.27-3.33-5.02zM14.13 4.36c.87-1.05 1.46-2.51 1.3-3.96-1.25.05-2.77.83-3.67 1.88-.81.93-1.51 2.42-1.32 3.85 1.39.11 2.82-.71 3.69-1.77z"/></svg>
              Continue with Apple
            </button>
          </form>
        )}
